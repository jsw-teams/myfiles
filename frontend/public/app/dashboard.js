...
const L = {
  'zh-CN': {
    ...
    yes: '是', no: '否', createUser: '创建用户', role: '角色', disabled: '已停用', active: '可用', enable: '启用', disable: '停用', passwordOptional: '新密码（留空不改）', allowRegistration: '允许自主注册', ssoEnabled: '启用 SSO 登录', ssoSecretPlaceholder: '留空则不修改现有 SSO 密钥',
  ...
  en: {
    ...
    yes: 'Yes', no: 'No', createUser: 'Create user', role: 'Role', disabled: 'Disabled', active: 'Active', enable: 'Enable', disable: 'Disable', passwordOptional: 'New password (leave blank to keep)', allowRegistration: 'Allow self-registration', ssoEnabled: 'Enable SSO sign-in', ssoSecretPlaceholder: 'Leave blank to keep the current SSO secret',
...
const settingFields = [
  ...
  ['auth', 'auth.allowRegistration', '允许自主注册', 'Allow self-registration', 'bool'], ['auth', 'auth.ssoEnabled', '启用 SSO 登录', 'Enable SSO sign-in', 'bool'], ['auth', 'sso.clientName', 'SSO 提供方名称', 'SSO provider name', 'text'], ['auth', 'sso.loginUrl', 'SSO 登录地址', 'SSO login URL', 'text'], ['auth', 'sso.accountBaseUrl', 'SSO 账户 API 地址', 'SSO account API URL', 'text'], ['auth', 'sso.meUrl', 'SSO 当前用户 API', 'SSO current-user API', 'text'], ['auth', 'sso.clientId', 'SSO Client ID', 'SSO Client ID', 'text'], ['auth', 'sso.clientSecret', 'SSO Client Secret', 'SSO Client Secret', 'ssoSecret'], ['auth', 'sso.redirectUri', 'SSO 回调地址', 'SSO redirect URI', 'text'], ['auth', 'sso.scopes', 'SSO Scopes', 'SSO scopes', 'textarea'],
...
function fieldHTML(key, label, value, type, disabled) {
  ...
  if (type === 'ssoSecret') return `<label class="field-block"><span>${escapeHtml(label)}</span><input ${attrs} type="password" autocomplete="new-password" placeholder="${escapeAttr(L.ssoSecretPlaceholder)}" value="" /></label>`;
...
async function saveSettingsLite(event) { event.preventDefault(); if (!state.me?.myfilesPermissions?.settingsWrite) return; const form = event.currentTarget; const body = {}; new FormData(form).forEach((raw, key) => { if (!key) return; body[key] = normalizeSetting(key, raw); }); const alert = document.querySelector('#settings-alert'); try { const json = await api('/api/admin/settings', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }); state.settings = json.settings || state.settings; assertSavedSettings(body, state.settings); renderSettingsGroup(); alert.className = 'pixel-alert ok'; alert.textContent = L.saved; } catch (error) { alert.className = 'pixel-alert'; alert.textContent = `${L.saveFailed}: ${error.message}`; } }
function normalizeSetting(key, raw) { const value = String(raw ?? '').trim(); if (['upload.allowAnonymous', 'file.defaultPublic', 'file.defaultRequireConfirm', 'auth.allowRegistration', 'auth.ssoEnabled'].includes(key)) return value === 'true'; if (['upload.maxMB', 'security.sessionTtlHours', 'audit.retentionDays', 'storage.timeoutSeconds'].includes(key)) return Number(value || 0); if (key === 'upload.allowedMimeTypes' || key === 'sso.scopes') return value.split(/\n|,/).map((item) => item.trim()).filter(Boolean); if (key === 'storage.r2Endpoint') return stripR2BucketFromEndpoint(value, document.querySelector('[name="storage.r2Bucket"]')?.value || ''); return value; }
function stripR2BucketFromEndpoint(value, bucket) { try { const url = new URL(value); const cleanBucket = String(bucket || '').replace(/^\/+|\/+$/g, ''); const parts = url.pathname.split('/').filter(Boolean); if (cleanBucket && parts.at(-1) === cleanBucket) parts.pop(); url.pathname = parts.length ? '/' + parts.join('/') : ''; return url.toString().replace(/\/$/, ''); } catch { return value.replace(/\/+$/, ''); } }
function assertSavedSettings(body, settings) { for (const [key, expected] of Object.entries(body)) { if (key.endsWith('Secret') && expected === '') continue; if (!Object.prototype.hasOwnProperty.call(settings || {}, key)) continue; const actual = settings[key]?.value ?? settings[key]; if (Array.isArray(expected)) { if (JSON.stringify(expected) !== JSON.stringify(actual || [])) throw new Error(`${key} not saved`); } else if (String(actual) !== String(expected)) throw new Error(`${key} not saved`); } }
